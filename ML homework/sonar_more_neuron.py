...
alpha=0.8
dep=2
filename='C:\\Users\\Asus\\Desktop\\机器学习\\方向一数据\\sonar.csv'
label="sonar"
title="改进-迭代次数与准确性的关系图-sonar"
save_png='改进-迭代次数与准确性的关系图-sonar.png'
color='g'
...
def network(dep):
    model=Sequential([
    layers.Dense(60, activation=tf.nn.leaky_relu), 
    layers.Dense(60, activation=tf.nn.leaky_relu), 
    layers.Dense(dep)])
...
def train(db_train,model,optimizer,dep):
    for step, (x, y) in enumerate(db_train):
        with tf.GradientTape() as tape:
            ...
            s=[]
            for q in range(len(model.trainable_variables)):        
                s.append(tf.nn.l2_loss(model.trainable_variables[q]))

acctitle="迭代次数与准确性的关系图-Simple_Newton"
accsave='迭代次数与准确性的关系图-Simple_Newton.png'
losstitle="迭代次数与误差的关系图-Simple_Newton"
losssave='迭代次数与误差的关系图-Simple_Newton.png'
labell='Simple_Newton'
...
with tf.GradientTape() as tape:
    y = tf.one_hot(sample[1], depth=3)
    out=model(sample[0])
    loss=tf.square(y - out)
    loss = tf.reduce_mean(loss)
grads_initial = tape.gradient(loss, model.trainable_variables)
...
for epoch in range(20000):
    ...
    for step, (x, y) in enumerate(db_train):
        ...
        loss = tf.reduce_mean(loss)
        #Simple_Newton
        grads_simple_newton=list()
        for i in range(len(model.trainable_variables)): 
                grads_simple_newton.append(loss/grads_initial[i])

        for j in range(len(model.trainable_variables)): 
            grads_simple_newton[j]=tf.clip_by_value(grads_simple_newton[j],-1e5,1e5)
        
        optimizer.apply_gradients(zip(grads_simple_newton, model.trainable_variables))
...
plt.plot(x,accc,c=seaborn.xkcd_rgb['olive drab'],label=labell)
plt.xlabel("迭代次数")
plt.ylabel("准确性")
plt.legend(loc=4) 
plt.title(acctitle)
...
plt.plot(x,losss,c=seaborn.xkcd_rgb['medium blue'],label=labell)
plt.xlabel("迭代次数")
plt.ylabel("误差")
plt.legend(loc=4) 
plt.title(losstitle)
